...
sh -c 'rsync -r -a -v -u -e "ssh -i $SSH_KEY_PRIVATE_PATH -o UserKnownHostsFile=$KNOWN_HOSTS_PATH" --delete --exclude .ssh $* $GITHUB_WORKSPACE $HOST_USER@$HOST_NAME:$HOST_PATH'
